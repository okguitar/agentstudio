...
13. [系统设置](#系统设置)
    ...
    - [供应商管理](#供应商管理)
    - [全局记忆](#全局记忆)
    - [MCP Admin Server](#mcp-admin-server)
    - [遥测与分析](#遥测与分析)
...
首页是 AgentStudio 的门户页面，展示了产品的核心价值和特性。
...
4. **核心特性展示**
   - 本地的 Agent 工作台：数据完全私有
   - Web 版交互体验：友好的界面
   - Claude Agent SDK 全家桶：MCP、技能系统、自定义命令等

5. **定时任务调度**
   - 让 Agent 按计划自动执行
   - 典型应用场景：每日日报、代码审查、周会纪要等
...
8. **对比表格**
   - AgentStudio vs Claude Code 功能对比
...
仪表板是您进入工作台后看到的第一个页面，提供了快速开始工作的入口。
...
项目设置允许您为每个项目配置默认的模型供应商和模型：

![项目设置对话框](screenshots/project-settings-dialog.png)

#### 功能说明

- **默认供应商**：为项目指定默认的 AI 模型供应商
- **默认模型**：为项目指定默认使用的 AI 模型
- **配置优先级**：系统会按照以下优先级应用模型配置：
  1. 聊天时手动选择（最高优先级）
  2. 项目默认设置（此处配置）
  3. 供应商首选模型
  4. 系统默认配置
...
定时任务管理页面用于配置 Agent 的自动执行计划，让 AI 助手按照预设的时间规则自动执行任务。

![定时任务概览](screenshots/scheduled-tasks-overview.png)
...
![创建定时任务对话框](screenshots/scheduled-tasks-create-dialog.png)
...
2. **执行配置**
   - **执行 Agent**（必填）：选择执行任务的 AI 助手
   - **项目路径**（必填）：Agent 执行时的工作目录
   - **覆盖模型配置**：可选择是否使用不同于项目默认的模型
...
3. **调度规则**
   ...
   | 类型 | 说明 | 示例 |
   |------|------|------|
   | **间隔执行** | 固定时间间隔重复执行 | 每 30 分钟 |
   | **Cron 表达式** | 使用 Cron 语法精确控制 | `0 9 * * 1-5`（工作日 9 点） |
   | **仅执行一次** | 在指定时间执行一次 | 2026/1/20 10:00:00 |

4. **触发消息**
   - **发送给 Agent 的消息**（必填）：任务触发时发送给 AI 的指令
   - 例如："请检查代码质量并生成报告"
...
点击任务行的 **"执行历史"** 按钮，查看任务的历史执行记录：

![执行历史](screenshots/scheduled-tasks-execution-history.png)
...
MCP Admin Server 允许外部 AI 工具（如 Cursor、Claude Desktop）通过 MCP 协议管理您的 AgentStudio：

![MCP Admin Server](screenshots/mcp-admin-settings.png)
...
3. **可用管理工具**
   ...
   - `get_agent`：获取助手详情
   - `update_agent`：更新助手配置
   - `toggle_agent_tool`：启用/禁用助手工具
   - `list_mcp_servers`：列出所有 MCP 服务
   - `add_mcp_server`：添加新 MCP 服务
   - `remove_mcp_server`：删除 MCP 服务
   ...
   - `get_default_provider`：获取默认供应商
   - `set_default_provider`：设置默认供应商
...
4. **开始使用**
   - 在 Cursor 中即可调用 AgentStudio 的管理功能
   - 例如：让 Claude 帮你创建项目、配置助手等
...
![遥测与分析](screenshots/telemetry-settings.png)
...
2. **我们收集的数据**
   - ✓ **匿名安装 ID**：随机 UUID，不与个人身份关联
   - ✓ **应用版本和操作系统**：帮助了解使用分布
   - ✓ **功能使用统计**：不包含内容或个人数据
   - ✓ **错误报告**：脱敏的错误信息（文件路径已移除）
